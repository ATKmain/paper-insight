...
import { PaperDocument } from "~/types/document";

export const useMultiplePdfs = (pdfs: PaperDocument[]) => {
  ...
  const isActivePdf = (file: PaperDocument) => {
    return file.url == activePdfUrl;
  };

  const handlePdfFocus = (file: PaperDocument) => {
